...
campagneApp <- function() {
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$auth_drive, {
      if (!googledrive::drive_has_token()) {
        googledrive::drive_auth(email = FALSE) # Will open browser to authenticate
      }
    ...
    observeEvent(input$add_permission, {
      ...
      if (level == "zone de sante" && (input$perm_province == "" || input$perm_antenne == "" || input$perm_zs == "")) {
        ...
        if (input$perm_zs == "") {
          missing_fields <- c(missing_fields, "Zone de Sant\u00e9")
        }
      }
    ...
    campaign_files <- reactiveVal(NULL)

    observeEvent(input$selected_campaign_monitoring, {
      masques <- googledrive::drive_ls(
        path = input$selected_campaign_monitoring,
        type = "spreadsheet",
        recursive = TRUE
      ) |>
        googledrive::drive_reveal(what = "modifiedTime") |>
        googledrive::drive_reveal(what = "webViewLink") |>
        dplyr::mutate(
          modified_time = as.Date(modified_time),
          last_updated = as.numeric(floor(difftime(Sys.Date(),
            modified_time,
            units = "days"
          )))
        ) |>
        dplyr::mutate(
          last_updated = as.integer(last_updated),
          province = stringr::str_match(name, ".*_PROV_([^_]+)_.*")[, 2],
          antenne = stringr::str_match(name, ".*_AN_([^_]+)_*")[, 2],
          zs = stringr::str_match(name, ".*_ZS_([^_]+)$")[, 2]
        ) |>
    ...
    # observeEvent(input$update_time_click, {
    #
    #   print(input$update_time_click$x)
    #
    #   # Get the options for clicking
    #   options <- campaign_files() |>
    #     dplyr::pull(last_updated) |>
    #     unique()
    #
    #   if (!is.null(input$update_time_click) &
    #       (floor(input$update_time_click) %in% options)) {
    #     filtered_table({campaign_files() |>
    #                      dplyr::filter(last_updated == floor(input$update_time_click))
    #         })
    #   } else {
    #     filtered_table(campaign_files())
    #   }
